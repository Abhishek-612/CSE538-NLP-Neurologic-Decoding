...
def adjust_logits_during_generation(logits, **kwargs):
    return logits
...
def _generate_beam_search(
    ...
    while cur_len < max_length:
        ...
        print(position_ids.shape,position_ids[:, -1].unsqueeze(-1).shape)
        if past:
        ...
        #print('Shape - ',model_inputs["position_ids"].shape,past.shape if past else '-',position_ids.shape)
        print(model_inputs["position_ids"].shape)
        ...
        if self.config.is_encoder_decoder and do_sample is False:
            next_token_logits = adjust_logits_during_generation(
                logits = next_token_logits, cur_len=cur_len, max_length=max_length
            )
...
def _reorder_cache(past: Tuple, beam_idx: Tensor) -> Tuple[Tensor]:
    ...
    print(past[0][0].index_select(0,beam_idx).shape)
    print('*****')
    return tuple(layer_past[0].index_select(0,beam_idx) for layer_past in past)
